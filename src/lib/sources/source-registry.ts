...
export const SOURCE_REGISTRY: SourceEntry[] = [
  ...
  },
  {
    ...
    costRisk: "none",
    legalRisk: "medium",
    pipelineUse: "Provider built (DeliverooAdapter.importRealDiscovery/.importRealDetail on parse-real.ts) and persistence-capable via the same commit_import_batch() transaction the import screen uses. A real public-flow discovery path is validated and repeatable (docs/74, docs/76) but detail-level enrichment (needed for postcode/address/phone) triggers Cloudflare protection at even 3-way concurrency — not yet production-safe.",
    liveEnabled: false,
    safeTonight: true,
    blocksExport: false,
    nextAction: "Owner decision needed: authorise wiring validated discovery into a scheduled adapter (recurring load + ToS posture — a bigger decision than a one-off manual check), AND determine a detail-enrichment approach that does not trigger bot protection (untested: whether strict sequential — no concurrency at all — requests avoid it; not attempted this session per the no-retry-after-challenge rule). thirdwatch/deliveroo-scraper remains disqualified (evaluated 2026-07-21, $0 spent).",
    notes: "No scraping / login / anti-bot bypass / bulk menu-price-review extraction. 2026-07-21: discovery validated twice (150/150 records, no challenge, docs/74 + docs/76). Detail enrichment: single sequential request succeeded (docs/74); 3 concurrent requests triggered a genuine Cloudflare challenge (docs/76) — correctly stopped, not bypassed, not retried. UB1 pilot run: 150 raw records captured, 0 canonical records persisted this run (all classified honestly unverifiable — postcode confirmation blocked by the challenge, not fabricated).",
    adapterImplemented: true,
    sourceAuthorised: false,
