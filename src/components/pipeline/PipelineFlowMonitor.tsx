...
const META: Record<StageId, Meta> = {
  ...
  fetch_just_eat: { kind: "source", desc: "Pull live Just Eat listings for each outcode (server-side public endpoint).", rules: ["Server-side only — no scraping/login/proxies", "Capped calls + delay", "Classify each record's territory"], now: (p) => `Pulling live Just Eat listings for ${p.join(", ")}…` },
