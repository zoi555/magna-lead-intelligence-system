...
const META: Record<StageId, Meta> = {
  ...
  fetch_just_eat: { kind: "source", desc: "Pull live Just Eat listings for each postcode district (server-side public endpoint).", rules: ["Server-side only — no scraping/login/proxies", "Capped calls + delay", "Classify each record's territory"], now: (p) => `Pulling live Just Eat listings for ${p.join(", ")}…` },
  platform_discovery: { kind: "source", desc: "Collect public platform evidence. Just Eat live; Deliveroo/Uber Eats are evidence-only (search URL or imported CSV).", rules: ["No login/captcha/proxy/anti-bot bypass", "Deliveroo/Uber not scraped — evidence only", "Aggregate rating/review only — no review text"], now: () => "Collecting public platform evidence (Just Eat / Deliveroo / Uber Eats)…" },
  source_fan_in: { kind: "transform", desc: "Merge FSA + Just Eat into one candidate set; enrich FSA with real platform data; promote in-area platform-only businesses.", rules: ["Match by name+postcode / proximity", "FSA-only + platform-only both kept", "Dedupe happens downstream"], now: () => "Fanning in FSA + Just Eat into one candidate set…" },
  ...
  customer_exclusion: { kind: "filter", desc: "Classify against the customer master (Active/Dormant/Former/Possible/New Prospect).", rules: ["Exact code/id/phone/name+postcode → exclude", "Postcode-only → hold (never auto-exclude)", "Only New Prospect Candidate proceeds"], now: () => "Excluding Active / Dormant / Former customer accounts…" },
  companies_house_status_gate: { kind: "gate", desc: "Match each lead to Companies House and gate on company status.", rules: ["Live only with key+enabled+cap (server-side)", "High-confidence dissolved → hold", "No match → continue (sole traders)"], now: () => "Checking Companies House status (hold dissolved companies)…" },
  companies_house_directors_enrichment: { kind: "enrich", desc: "Fetch officers/directors for high-confidence company matches (INTERNAL research only).", rules: ["High-confidence matches only", "Never in telesales-safe export", "Cap-aware"], now: () => "Fetching directors/officers for confirmed companies (internal)…" },
  companies_house_financials_stage: { kind: "enrich", desc: "Discover accounts filings, best-effort XBRL extraction, financial ratios + health/risk (INTERNAL only).", rules: ["High-confidence matches only", "Never invent values — null if absent", "Raw financials never in telesales export", "Cap-aware; PDF-only → manual review"], now: () => "Discovering accounts filings + financial risk (internal)…" },
  google_places_enrichment: { kind: "enrich", desc: "Fill phone / website / rating / review count via Google Places (paid, capped, prioritised). Disabled → contact enrichment incomplete.", rules: ["Off unless key + enabled + cap", "Server-side only; field-masked", "Aggregate review count only — no review text", "Never blocks the pipeline"], now: () => "Enriching phone / website / reviews via Google Places (if enabled)…" },
  data_completeness: { kind: "transform", desc: "Score each lead 0–100 on data completeness (identity, contact, location, source evidence).", rules: ["Bands: ready 80+ / usable 60–79 / weak 40–59 / poor <40", "Lists missing fields + which source would fill them", "Target 80%+ for a sales-ready dataset"], now: () => "Scoring data completeness per lead…" },
  linkedin_research_queue_generation: { kind: "enrich", desc: "Generate LinkedIn/Google manual research queues (public search URLs only).", rules: ["NO scraping / login / cookies / automation", "Manual research queue only", "Not in telesales export"], now: () => "Generating LinkedIn/Google manual research queues…" },
  delivery_platform_presence_summary: { kind: "enrich", desc: "Summarise delivery-platform presence. Just Eat is live; Uber Eats/Deliveroo remain manual/import evidence.", rules: ["Just Eat = live public endpoint", "Uber/Deliveroo = manual/import only — no scraping", "Not a blocker"], now: () => "Summarising Just Eat / Uber Eats / Deliveroo presence…" },
  commercial_calculation: { kind: "transform", desc: "Estimate commercial value (ESTIMATED / ASSUMPTION-BASED). Workings internal only.", rules: ["Placeholder assumptions — not quotes", "Only bands reach the sales list", "Raw workings internal audit only"], now: () => "Estimating commercial value (assumption-based)…" },
  ...
  generate_tomorrow_sales_exports: { kind: "export", desc: "Write tomorrow's sales list + audit/exclusion exports.", rules: ["Sales list = New Prospect Candidates only", "Safe fields only (no score/internals)", "Labelled if customer list not loaded"], now: () => "Writing tomorrow's sales list + audit exports…" },
};
