...
async function main() {
  ...
  // ============================================================
  {
    ...
    await queueJustEatExecution(repo, run.id);
  ...
  // ============================================================
  {
    ...
    await queueJustEatExecution(repo, queuedRun.id);
    ...
    const { run: toFail } = await saveRun(repo, { tenant_id: "t", name: "r6-tofail", territory_input: "UB1" }, geoRef);
    await queueJustEatExecution(repo, toFail.id);
    ...
    const { run: stuckRun } = await saveRun(repo, { tenant_id: "t", name: "r6-stuck", territory_input: "UB1" }, geoRef);
    await queueJustEatExecution(repo, stuckRun.id);
  ...
  // ============================================================
  {
    ...
    const zExec = await queueJustEatExecution(repo, zeroRun.id);
    ...
    await queueJustEatExecution(repo, incompleteRun.id);
